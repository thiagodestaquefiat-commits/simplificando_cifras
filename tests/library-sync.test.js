...
(async()=>{
 ...
 assert.match(sw,/simplificando-cifras-v110-event-convergence-hotfix/);assert.match(sw,/song-repository\.js\?v=6/);assert.match(sw,/library-sync\.js\?v=9/);assert.match(sw,/event-repository\.js\?v=5/);assert.match(sw,/event-collaboration-client\.js\?v=7/);assert.match(sw,/import-library\.js\?v=1/);assert.doesNotMatch(sw,/localStorage\.(?:clear|removeItem)/,'atualização do cache não apaga biblioteca');
