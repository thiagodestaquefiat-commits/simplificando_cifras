...
(async()=>{
 ...
 assert.match(html,/openLibraryMigrationPrompt\(state\)/);assert.match(html,/Encontramos .* neste dispositivo/);assert.match(html,/Salvar na minha conta/);assert.match(html,/Agora não/);assert.match(html,/confirmLibraryMigration\(\)[\s\S]*?exportarBiblioteca\(\)[\s\S]*?markBackupReady\(\)[\s\S]*?syncNow\(\)/,'confirmação simples gera backup antes de sincronizar');assert.doesNotMatch(html,/setTimeout\(\(\)=>openLibrarySync\(\),0\)/,'bootstrap e login nunca abrem o painel técnico');
 assert.match(sw,/simplificando-cifras-v100-personal-library-background/);assert.match(sw,/song-repository\.js\?v=3/);assert.match(sw,/library-sync\.js\?v=8/);assert.match(sw,/import-library\.js\?v=1/);assert.doesNotMatch(sw,/localStorage\.(?:clear|removeItem)/,'atualização do cache não apaga biblioteca');
