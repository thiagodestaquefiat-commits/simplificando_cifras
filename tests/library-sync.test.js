...
function device(userId,initial,{online=true,consent=false,remoteFormat='camel',migrationCandidate=false,awaitingRemoteOnboarding=false,demos=[]}={}){
 ...
 let songs=structuredClone(initial),authUser=userId,authListener,requests=[],batches=[],statusEvents=[],ownerConfirmed=false,deactivateCalls=0;const networkListeners={};
...
(async()=>{
 ...
 const anonymous=device('',five);assert.equal(anonymous.sync.getStatus().phase,'unauthenticated');assert.equal(anonymous.deactivateCalls,0,'bootstrap sem login preserva a biblioteca visitante');anonymous.sync.schedule();await settle();assert.equal(anonymous.requests.includes('POST'),false,'sem login não tenta enviar');assert.equal(remote.size,0,'sem login não envia');
...
assert.match(html,/Neste dispositivo/);assert.match(html,/Na nuvem/);assert.match(html,/Para enviar/);assert.match(html,/Para baixar/);assert.match(html,/Somente neste dispositivo/);assert.match(html,/Baixar diagnóstico/);assert.match(html,/Conflitos/);assert.match(html,/Entrar com Google/);assert.match(html,/Opções avançadas/);assert.match(html,/Tudo sincronizado/);assert.match(html,/Você está offline/);assert.match(html,/<div class="topbar-title">ROUDY<\/div>/);assert.doesNotMatch(html,/<button[^>]+onclick="exportarBiblioteca\(\)"/);assert.doesNotMatch(html,/<button[^>]+id="library-sync-btn"/,'painel técnico não aparece na navegação normal');assert.match(html,/downloadSyncDiagnostics\(\)[\s\S]*?await librarySync\.review\(\)\.catch\(\(\)=>null\);await librarySync\.refreshServerAudit\(\)/,'diagnóstico interno permanece disponível no código');assert.match(html,/!state\.identityBlocked/,'guard de owner não confirmado permanece intacto');
 ...
 assert.match(sw,/simplificando-cifras-v144-ai-integrated-release/);assert.match(sw,/tablature\.js\?v=1/);assert.match(sw,/song-model\.js\?v=5/);assert.match(sw,/demo-library\.js\?v=3/);assert.match(sw,/study-metronome\.js\?v=2/);assert.match(sw,/study-metronome\.css\?v=3/);assert.match(sw,/song-repository\.js\?v=13/);assert.match(sw,/library-sync\.js\?v=16/);assert.match(sw,/event-repository\.js\?v=6/);assert.match(sw,/event-collaboration-client\.js\?v=11/);assert.match(sw,/app-auth\.js\?v=11/);assert.match(sw,/ui-i18n\.js\?v=10/);assert.match(sw,/tuner\.js\?v=2/);assert.match(sw,/smart-scroll\.js\?v=2/);assert.match(sw,/sync-realtime\.js\?v=1/);assert.match(sw,/event-chat\.js\?v=2/);assert.match(sw,/import-library\.js\?v=1/);assert.doesNotMatch(sw,/localStorage\.(?:clear|removeItem)/,'atualização do cache não apaga biblioteca');
