...
(async()=>{
 ...
 assert.match(sw,/simplificando-cifras-v120-identity-ownership-fix/);assert.match(sw,/study-metronome\.js\?v=2/);assert.match(sw,/study-metronome\.css\?v=3/);assert.match(sw,/song-repository\.js\?v=8/);assert.match(sw,/library-sync\.js\?v=11/);assert.match(sw,/event-repository\.js\?v=5/);assert.match(sw,/event-collaboration-client\.js\?v=8/);assert.match(sw,/app-auth\.js\?v=8/);assert.match(sw,/import-library\.js\?v=1/);assert.doesNotMatch(sw,/localStorage\.(?:clear|removeItem)/,'atualização do cache não apaga biblioteca');
