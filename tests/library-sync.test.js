...
(async()=>{
 ...
 const preexisting=device('user-a',[song(100),song(101),song(102)],{migrationCandidate:true});await settle();assert.equal(preexisting.songs.length,8,'biblioteca local ainda não associada é preservada e combinada');assert.equal(preexisting.sync.getStatus().pending,3);assert.equal(preexisting.sync.getStatus().phase,'needs-consent');
 ...
 assert.match(html,/openLibraryMigrationPrompt\(state\)/);assert.match(html,/Salvar suas músicas/);assert.match(html,/Encontramos .* neste aparelho/);assert.match(html,/Salvar minhas músicas/);assert.match(html,/Agora não/);assert.match(html,/confirmLibraryMigration\(\)[\s\S]*?exportarBiblioteca\(\{quiet:true\}\)[\s\S]*?markBackupReady\(\)[\s\S]*?syncNow\(\)/,'confirmação simples gera backup silencioso antes de sincronizar');assert.doesNotMatch(html,/setTimeout\(\(\)=>openLibrarySync\(\),0\)/,'bootstrap e login nunca abrem o painel técnico');
 assert.match(sw,/simplificando-cifras-v101-personal-library-simple/);assert.match(sw,/song-repository\.js\?v=3/);assert.match(sw,/library-sync\.js\?v=8/);assert.match(sw,/import-library\.js\?v=1/);assert.doesNotMatch(sw,/localStorage\.(?:clear|removeItem)/,'atualização do cache não apaga biblioteca');
