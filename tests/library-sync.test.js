...
(async()=>{
 ...
 assert.match(sw,/simplificando-cifras-v103-personal-events-repertoire/);assert.match(sw,/song-repository\.js\?v=4/);assert.match(sw,/library-sync\.js\?v=8/);assert.match(sw,/event-repository\.js\?v=3/);assert.match(sw,/event-collaboration-client\.js\?v=5/);assert.match(sw,/import-library\.js\?v=1/);assert.doesNotMatch(sw,/localStorage\.(?:clear|removeItem)/,'atualização do cache não apaga biblioteca');
