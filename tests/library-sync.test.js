...
(async()=>{
 ...
 assert.match(html,/Encontramos .* neste dispositivo/);assert.match(html,/Baixar backup completo/);assert.match(html,/Salvar na minha conta/);assert.match(html,/Agora não/);assert.doesNotMatch(html,/promptedLibraryMigrationOwner|setTimeout\(\(\)=>openLibrarySync\(\),0\)/,'bootstrap, reload e login não abrem o modal automaticamente');
 assert.match(sw,/simplificando-cifras-v99-sync-modal-manual/);assert.match(sw,/song-repository\.js\?v=3/);assert.match(sw,/library-sync\.js\?v=7/);assert.match(sw,/import-library\.js\?v=1/);assert.doesNotMatch(sw,/localStorage\.(?:clear|removeItem)/,'atualização do cache não apaga biblioteca');
