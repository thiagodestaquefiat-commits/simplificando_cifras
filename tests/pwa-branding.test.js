...
assert.match(serviceWorker, /simplificando-cifras-v134-single-ai-button/);
...
assert.match(serviceWorker, /event-collaboration-client\.js\?v=10/);
assert.match(serviceWorker, /js\/ai\/harmonic-summary-client\.js/);
assert.match(serviceWorker, /js\/editor\/song-editor\.js/);
assert.match(indexHtml, /js\/editor\/song-format\.js\?v=9/);
assert.match(serviceWorker, /js\/editor\/song-format\.js\?v=9/);
...
assert.match(serviceWorker, /js\/demo-library\.js\?v=3/);
assert.match(serviceWorker, /js\/song-repository\.js\?v=11/);
assert.match(serviceWorker, /js\/library-sync\.js\?v=15/);
...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator(".music-item").count(), 4, "restauração idempotente não duplica as quatro demos");
    assert.equal(await page.evaluate(() => localStorage.getItem("cifras_setlists_v1")), persistedPlaylists);
    assert.equal(await page.evaluate(() => localStorage.getItem("cifras_favoritos_v1")), persistedFavorites);
    await page.getByRole("button", { name: "Voltar para minha conta", exact: true }).click();
    ...
    assert.equal(await page.locator(".music-item").count(), 4);
    assert.equal(await page.evaluate(() => localStorage.getItem("cifras_setlists_v1")), persistedPlaylists);
    assert.equal(await page.evaluate(() => localStorage.getItem("cifras_favoritos_v1")), persistedFavorites);
