...
const saved = window.songFormat.toLegacy(model, spotify); assert.equal(saved.spotifyTrackId, "track-1"); assert.equal(saved.spotifyUri, "spotify:track:track-1"); assert.equal(saved.album, "Ideologia"); assert.equal(saved.blocos[0].l, "Refrão");
console.log("ai-song-integration.test.js: OK (API central, Song Spotify, rascunho e metadados preservados)");
