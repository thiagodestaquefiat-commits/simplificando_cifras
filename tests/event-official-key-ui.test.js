...
(async () => {
  ...
  try {
    for (const viewport of [{width:390,height:844},{width:768,height:1024}]) {
      ...
      await page.route('**/api/collaboration/**', route => route.fulfill({status:404,contentType:'application/json',body:JSON.stringify({erro:{codigo:'nao_publicado',mensagem:'Backend de teste offline'}})}));
      ...
      await page.evaluate(() => {
        ...
        setlists.push(eventModel.create({id:'event-key',title:'Evento do teste',leaderId:appCurrentUser.id,members:[{...appCurrentUser,isLeader:true},{id:'other-member',name:'Outra pessoa',role:'Vocal'}],repertoire:[{id:'event-key-item',songId:'event-key-song',shared:{notes:'Entrada suave',chordSheet:'C F G\nUma letra'}}]}));
        openSD('event-key');
      });
      ...
      assert.equal(await page.evaluate(() => findEvent('event-key-member').repertoire[0].shared.key),'Db');
