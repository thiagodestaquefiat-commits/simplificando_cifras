...
(async()=>{
  ...
  try{
    for(const viewport of [{width:360,height:800},{width:390,height:844},{width:412,height:915},{width:844,height:390},{width:768,height:1024},{width:1366,height:768}]){
      ...
      await page.evaluate(()=>adjustStageScrollSpeed(-1));assert.equal(await value(stage),'0,10x');
      ...
      assert.equal(await page.evaluate(()=>document.getElementById('view-detail').scrollTop),before);
      await page.evaluate(()=>advanceScroll(100));
      assert.equal(await page.evaluate(()=>document.getElementById('view-detail').scrollTop),before+1,'minimum speed accumulates fractional pixels');
      const timer=await page.evaluate(()=>scrollTimer);
      for(let i=0;i<38;i++)await plus(stage).click();
