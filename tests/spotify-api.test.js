...
(async () => {
  ...
  await window.spotifyApi.startPlayback("device-1", "spotify:track:track-1");
  ...
  assert.deepEqual(JSON.parse(requests[2].options.body), { uris: ["spotify:track:track-1"] });
  ...
  assert.match(requests[4].url, /\/me\/player\/repeat\?state=track&device_id=device-1$/);
