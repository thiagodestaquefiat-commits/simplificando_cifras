(function (global) {
  ...
  function renderMetadata(model) {
    const panel = el("section", "se-panel"); panel.appendChild(el("h3", "", model.aiGenerated ? "Revisão do resumo" : "Dados da música")); const grid = el("div", "se-grid");
    grid.append(field("Título", model.title, "title", "text", {wide:true}), field("Artista", model.artist, "artist", "text", {wide:true}), field("Tom", model.currentKey, "currentKey"));
    if (!model.aiGenerated) grid.append(field("Tom original", model.originalKey, "originalKey"), field("Capotraste", model.capo, "capo", "number", {min:0,max:12}), field("BPM", model.bpm, "bpm", "number", {min:20,max:300}), field("Instrumento", model.instrument, "instrument", "select", {items:global.instrumentDefinitions.all.map((item)=>[item.id,item.name]),wide:true}), field("Origem", model.source, "source", "select", {items:[["manual","Manual"],["imported","Importada"],["ai","IA"],["existing","Existente"]]}), field("Status", model.status, "status", "select", {items:[["draft","Rascunho"],["published","Música salva"]]}), field("Observações", model.notes, "notes", "textarea", {wide:true}));
    else if (model.notes) { const details=el("details","se-ai-details");details.append(el("summary","","Ver detalhes"),field("Confiança, avisos e observações",model.notes,"notes","textarea",{wide:true}));panel.appendChild(details); }
  ...
  function shell(model, mode) {
    ...
    const main=el("main","song-editor-main");if(model.aiGenerated)main.appendChild(el("div","se-ai-notice",model.reviewedByUser?"Revisado":"Gerado por IA — revise antes de salvar."));main.appendChild(mode==="edit"?renderEdit(model):mode==="preview"?renderPreview(model):renderSummary(model));overlay.appendChild(main);
